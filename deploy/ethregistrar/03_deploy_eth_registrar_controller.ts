...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const ethOwnedResolver = await ethers.getContract('OwnedResolver', deployer)
  ...
  if (deployer !== deployer) {
    ...
    const tx = await c.transferOwnership(deployer)
    console.log(
      `Transferring ownership of ETHRegistrarController to ${deployer} (tx: ${tx.hash})...`,
    )
    await tx.wait()
  }
